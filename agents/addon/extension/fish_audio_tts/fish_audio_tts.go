...
func defaultFishAudioTTSConfig() fishAudioTTSConfig {
	return fishAudioTTSConfig{
		ApiKey:                   "",
		ModelId:                  "e58b0d7efca34eb38d5c4985e378abcb", // Trump
...
func (e *fishAudioTTS) textToSpeechStream(rteEnv rte.RteEnv, streamWriter io.Writer, text string) (err error) {
	latency := "normal"
	...
	if resp.StatusCode != http.StatusOK {
		slog.Error(fmt.Sprintf("Unexpected response status, status: %d", resp.StatusCode), logTag)
	...
	for {
		...
		if err != nil && err != io.EOF {
			slog.Error(fmt.Sprintf("Failed to read from response body, error: %s", err), logTag)
		...
		if writeErr != nil {
			slog.Error(fmt.Sprintf("Failed to write to streamWriter, error: %s", writeErr), logTag)
